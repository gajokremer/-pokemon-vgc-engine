...
class ChampionshipEcosystem:
    ...
    def __init__(
        ...
        self.league: BattleEcosystem = BattleEcosystem(
            self.meta_data, debug, render, n_battles, strategy, update_meta=True
        )
        self.debug = debug
        self.roster_ver = 0
    ...
    def __set_new_team(self, cm: CompetitorManager):
        try:
            cm.team = cm.competitor.team_build_policy.get_action(
                self.meta_data
            ).get_copy()
